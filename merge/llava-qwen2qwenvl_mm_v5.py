...
# --- 核心实现类 ---
# NEW: 引入全新的 R_E_DAMCPMMerger 类来实现新方法
class R_E_DAMCPMMerger:
    def __init__(self, args, device):
        self.args = args
        self.device = device
        self.output_dir = os.path.join("merged_models", f"r-e-damcpm-{args.mode}")
    ...
    def _cache_activations_raw(self, model_info, model_path, required_activations, dataset_raw):
        ...
        # 内存优化：存储运行总和、平方和、计数，用于计算均值和方差
        activation_stats = defaultdict(lambda: {
            "input_sum": None, "input_sum_sq": None, "input_count": 0,
            "output_sum": None, "output_sum_sq": None, "output_count": 0
        })
    
        # 内存优化：钩子函数直接更新总和和平方和，而不是追加到列表
            ...
            def hook_fn(module, args, kwargs, output):
                ...
                if "output" in req_act:
                    ...
                    if isinstance(out_tensor, torch.Tensor):
                        ...
                        current_sum_sq = torch.sum(t_reshaped**2, dim=0)
                        
                        if activation_stats[name]["output_sum"] is None:
                            activation_stats[name]["output_sum"] = current_sum
                            activation_stats[name]["output_sum_sq"] = current_sum_sq
                        else:
                            activation_stats[name]["output_sum"] += current_sum
                            activation_stats[name]["output_sum_sq"] += current_sum_sq
                        activation_stats[name]["output_count"] += t_reshaped.shape[0]
                ...
                if "input" in req_act:
                    ...
                    if isinstance(in_tensor, torch.Tensor):
                        ...
                        if activation_stats[name]["input_sum"] is None:
                            ...
                            activation_stats[name]["input_sum_sq"] = current_sum_sq
                        else:
                            activation_stats[name]["input_sum"] += current_sum
                            activation_stats[name]["input_sum_sq"] += current_sum_sq
                        activation_stats[name]["input_count"] += t_reshaped.shape[0]
            return hook_fn
        ...
        for name, stats in activation_stats.items():
            ...
            if stats["input_sum"] is not None and stats["input_count"] > 0:
                mean = stats["input_sum"] / stats["input_count"]
                var = (stats["input_sum_sq"] / stats["input_count"] - mean**2) * stats["input_count"] / (stats["input_count"] - 1) if stats["input_count"] > 1 else torch.zeros_like(mean)
                averaged_activations[name]["input_mean"] = mean
                averaged_activations[name]["input_var"] = var
            if stats["output_sum"] is not None and stats["output_count"] > 0:
                mean = stats["output_sum"] / stats["output_count"]
                var = (stats["output_sum_sq"] / stats["output_count"] - mean**2) * stats["output_count"] / (stats["output_count"] - 1) if stats["output_count"] > 1 else torch.zeros_like(mean)
                averaged_activations[name]["output_mean"] = mean
                averaged_activations[name]["output_var"] = var
    ...
    def stage2_analyze_neurons_and_get_masks(self):
        ...
        for key in tqdm(weights_A.keys(), desc="分析神经元"):
            ...
            try:
                # 计算稳定性系数 ρ
                sigma_X_A = torch.sqrt(activations['A'][module_name]['input_var'])
                mu_X_A = activations['A'][module_name]['input_mean']
                rho_A = 1 / (1 + torch.norm(sigma_X_A, p=1) / (torch.norm(torch.abs(mu_X_A), p=1) + 1e-8))
                
                sigma_X_B = torch.sqrt(activations['B'][module_name]['input_var'])
                mu_X_B = activations['B'][module_name]['input_mean']
                rho_B = 1 / (1 + torch.norm(sigma_X_B, p=1) / (torch.norm(torch.abs(mu_X_B), p=1) + 1e-8))
                
                # 计算正则化近似梯度
                delta_Y_A = activations['A'][module_name]['output_mean'] - activations['C'][module_name]['output_mean']
                g_approx_A = torch.outer(rho_A * delta_Y_A, activations['A'][module_name]['input_mean'])
                
                delta_Y_B = activations['B'][module_name]['output_mean'] - activations['C'][module_name]['output_mean']
                g_approx_B = torch.outer(rho_B * delta_Y_B, activations['B'][module_name]['input_mean'])
    ...
    def stage3_project_and_merge(self):
        ...
        for key in weights_A.keys():
            ...
            if 'layers' in parts:
                ...
                type_key = key.replace(f'layers.{layer_idx}.', 'layers.0.')
                groups[type_key].append(key)
                layer_indices[key] = layer_idx

        task_vectors_groups = {}
        for type_key, keys in groups.items():
            tau_list = []
            for key in keys:
                key_in_c = key.replace("model.language_model.", "model.")
                tau_B = weights_B[key_in_c] - weights_C[key_in_c]
                tau_list.append(tau_B)
            task_vectors_groups[type_key] = tau_list

        coord_weights = {}
        for type_key, tau_list in task_vectors_groups.items():
            bar_tau = sum(tau_list) / len(tau_list)
            bar_norm = torch.norm(bar_tau, p='fro')
            for i, tau in enumerate(tau_list):
                tau_norm = torch.norm(tau, p='fro')
                if tau_norm > 0 and bar_norm > 0:
                    cos_sim = torch.trace(tau.T @ bar_tau) / (tau_norm * bar_norm)
                else:
                    cos_sim = 0.0
                c_i = max(0, cos_sim.item())
                coord_weights[groups[type_key][i]] = c_i
        ...
        for key, mask_B in tqdm(non_conflict_masks.items(), desc="执行合并"):
            ...
            W_A, W_B, W_C = weights_A[key], weights_B[key_in_c], weights_C[key_in_c]
            d_i = activations_A[module_name]['input_mean'].to(self.device)  # 投影方向
            
            tau_B = (W_B - W_C).to(self.device)
            c_i = coord_weights.get(key, 1.0)  # 如果无协调，默认1.0
            tilde_tau_B = c_i * tau_B
            tau_prime = tilde_tau_B * mask_B.to(self.device)
            
            d_norm_sq = torch.sum(d_i * d_i) + 1e-8
            proj_scalar = (tau_prime.float() @ d_i.float()) / d_norm_sq
            tau_proj = torch.outer(proj_scalar, d_i.float())
            tau_ortho = tau_prime - tau_proj

            # 计算解析 λ*
            v_proj = tau_proj @ activations_A[module_name]['input_mean'].to(self.device)
            v_ortho = tau_ortho @ activations_A[module_name]['input_mean'].to(self.device)
            u = activations_B[module_name]['output_mean'].to(self.device) - activations_A[module_name]['output_mean'].to(self.device)
            
            V = torch.stack([v_proj, v_ortho]).T  # D x 2
            VT_V = V.T @ V + 1e-8 * torch.eye(2, device=self.device)
            VT_u = V.T @ u
            lambda_vec = torch.linalg.solve(VT_V, VT_u)
            lambda_proj, lambda_ortho = lambda_vec[0], lambda_vec[1]
            
            # 互信息近似调整
            sigma_tau_plus_YA = torch.sqrt(activations_B[module_name]['output_var'] + activations_A[module_name]['output_var']).to(self.device)
            sigma_tau = torch.sqrt(activations_B[module_name]['output_var']).to(self.device)
            log_det_tau_plus = torch.sum(torch.log(sigma_tau_plus_YA + 1e-8))
            log_det_tau = torch.sum(torch.log(sigma_tau + 1e-8))
            I = log_det_tau_plus - log_det_tau
            partial_I_lambda = (activations_B[module_name]['output_var'].mean() - activations_A[module_name]['output_var'].mean()) / (torch.norm(V, dim=0)**2 + 1e-8)
            lambda_proj += self.args.beta * partial_I_lambda[0] * I
            lambda_ortho += self.args.beta * partial_I_lambda[1] * I

            # 最终合并
            W_star = W_A.to(self.device).float() + lambda_proj * tau_proj + lambda_ortho * tau_ortho
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="使用R-E-DAMCPM进行高效、无需反向传播的模型合并。")
    ...
    parser.add_argument('--beta', type=float, default=0.1, help="泛化正则权重。")
    ...
    merger = R_E_DAMCPMMerger(args, device)
    merger.run_pipeline()