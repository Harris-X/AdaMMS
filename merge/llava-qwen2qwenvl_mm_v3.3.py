...
# --- 核心实现类 ---
# NEW: 引入全新的 RAGIWPMMerger 类来实现新方法
class RAGIWPMMerger:
    def __init__(self, args, device):
        self.args = args
        self.device = device
        self.output_dir = os.path.join("merged_models", f"r-agi-wpm-{args.mode}")
    ...
    def stage2_analyze_neurons_and_get_masks(self):
        """阶段二：计算近似SNIP并生成软掩码和精细权重。"""
        print("\n--- [阶段二: 近似SNIP神经元分析] ---")
        weight_cache_path = os.path.join(self.cache_dir, f"fine_weights_r{self.args.top_k_ratio}.pt")
        if os.path.exists(weight_cache_path) and not self.args.force_recompute:
            print("精细权重缓存文件已存在, 跳过。")
            return
        ...
        for key in tqdm(weights_A.keys(), desc="分析神经元"):
            ...
            try:
                ...
                # 计算冲突强度
                epsilon = 1e-8
                conflict_mask = mask_A & mask_B
                s_conflict = torch.zeros_like(tau_A)
                s_conflict[conflict_mask] = torch.abs(tau_A[conflict_mask] * tau_B[conflict_mask]) / (torch.abs(tau_A[conflict_mask]) * torch.abs(tau_B[conflict_mask]) + epsilon)
                
                # 软掩码
                m_prime_B = torch.zeros_like(mask_B, dtype=torch.float)
                m_prime_B[mask_B] = torch.exp(-s_conflict[mask_B])
                
                # 精细权重
                max_snip_B = snip_B.max() + epsilon
                w_B = (snip_B * m_prime_B) / max_snip_B
                
                fine_weights[key] = w_B
            except KeyError:
                print(f"警告: 模块 {module_name} 的激活数据不完整，跳过参数 {key}。")
                continue

        torch.save(fine_weights, weight_cache_path)
        print(f"精细权重计算完成并缓存至: {weight_cache_path}")

    def stage3_project_and_merge(self):
        """阶段三：执行加权投影合并。"""
        print("\n--- [阶段三: 加权投影合并] ---")
        
        print("加载所有权重、权重矩阵和方向向量...")
        weights_A = load_weights(self.args.base_model_path)
        weights_B_raw = load_weights(self.args.donor_model_path)
        weights_C = load_weights(self.args.original_model_path)
        weights_B = normalize_llm_keys(weights_B_raw, list(weights_C.keys())); del weights_B_raw
        ...
        weight_cache_path = os.path.join(self.cache_dir, f"fine_weights_r{self.args.top_k_ratio}.pt")
        fine_weights = torch.load(weight_cache_path)
        ...
        epsilon = 1e-8
        for key, w_B in tqdm(fine_weights.items(), desc="执行合并"):
            key_in_c = key.replace("model.language_model.", "model.")
            ...
            tau_prime_B = tau_B * w_B.to(self.device)
            
            d_i_norm_sq = torch.dot(d_i, d_i) + epsilon
            proj_scalar = torch.matmul(tau_prime_B.float(), d_i.float()) / d_i_norm_sq
            tau_B_proj = torch.outer(proj_scalar, d_i)
            tau_B_ortho = tau_prime_B - tau_B_proj
            
            # 计算最优比例
            v_proj = torch.matmul(tau_B_proj.float(), activations_A[module_name]['input'].to(self.device).float())
            v_ortho = torch.matmul(tau_B_ortho.float(), activations_A[module_name]['input'].to(self.device).float())
            u = activations_B[module_name]['output'].to(self.device).float() - activations_A[module_name]['output'].to(self.device).float()
            
            V = torch.stack([v_proj, v_ortho], dim=1)  # D x 2
            VT_V = torch.matmul(V.T, V)  # 2 x 2
            R = torch.diag(torch.tensor([self.args.gamma * torch.dot(v_proj, v_proj), 0.0]).to(self.device))
            VT_u = torch.matmul(V.T, u)  # 2
            
            mat_to_inv = VT_V + R + epsilon * torch.eye(2, device=self.device)
            lambda_vec = torch.matmul(torch.linalg.inv(mat_to_inv), VT_u)
            lambda_proj, lambda_ortho = lambda_vec[0], lambda_vec[1]

            # 最终合并
            W_star = W_A.to(self.device).float() + lambda_proj * tau_B_proj + lambda_ortho * tau_B_ortho
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="使用R-AGI WPM进行高效、无需反向传播的模型合并。")
    ...
    parser.add_argument('--mode', type=str, default="r-agi-wpm-default", help="为本次合并配置命名。")
    ...
    merger = RAGIWPMMerger(args, device)
    merger.run_pipeline()